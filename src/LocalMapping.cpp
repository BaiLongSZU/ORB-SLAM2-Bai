...
void LocalMapping::SearchInNeighbors()
{
    ...
    //白，把与当前帧相邻的一级、二级相邻帧全部找出，然后存入vpTargetKFs
    ...
    // 步骤2：将当前帧的MapPoints分别与一级二级相邻帧(的特征点)进行融合
    ...
    for(vector<KeyFrame*>::iterator vit=vpTargetKFs.begin(), vend=vpTargetKFs.end(); vit!=vend; vit++)
    {
        ...
        // 1.如果MapPoint能匹配关键帧的特征点，并且该点**有**对应的MapPoint，那么将两个MapPoint合并（选择观测数多的）
        // 2.如果MapPoint能匹配关键帧的特征点，并且该点**没有**对应的MapPoint，那么为该点添加MapPoint
    ...
    // 步骤3：将一级二级相邻帧的MapPoints分别与当前帧（的特征点）进行融合
    // 遍历每一个一级邻接和二级邻接关键帧
    ...
    //白，针对当前帧的一、二级相邻帧的MapPoints，所有有改变的MapPoints都与当前帧的特征点有关联，所以已经链接到当前帧的MapPoints队列中了，以当前帧的MapPoints队列为索引进行描述子更新即可对所有有改变的MapPoints描述子更新。
