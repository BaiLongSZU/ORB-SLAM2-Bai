...
void KeyFrame::UpdateConnections()
{
    ...
    for(map<KeyFrame*,int>::iterator mit=KFcounter.begin(), mend=KFcounter.end(); mit!=mend; mit++)
    {
	    ...
	    //白，在此只对关联帧对象增加“与本帧的关联度”，后面会对本帧增加“与关联帧的关联度”（此时，没有阈值筛选）。
    ...
    //===============3==================================
    {
        ...
        // 更新图的连接(权重)
	//白，此处应该有问题，KFcounter与vPairs大小不一致，vPairs是KFcounter筛选关联度之后的结果
...
float KeyFrame::ComputeSceneMedianDepth(const int q)
{
    ...
    vDepths.reserve(N);
    cv::Mat Rcw2 = Tcw_.row(2).colRange(0,3);	//白，与Z值相关的旋转变换列
    Rcw2 = Rcw2.t();				//白，为了做点乘，使向量的形式一致
    float zcw = Tcw_.at<float>(2,3);		//白，与Z值相关的平移量
    for(int i=0; i<N; i++)
    {
        if(mvpMapPoints[i])
        {
