...
int ORBmatcher::SearchForInitialization(Frame &F1, Frame &F2, vector<cv::Point2f> &vbPrevMatched, vector<int> &vnMatches12, int windowSize)
{
    ...
    vector<int> vnMatches21(F2.mvKeysUn.size(),-1);			//白，只是录入数据，但是没有使用过
    ...
    for(size_t i1=0, iend1=F1.mvKeysUn.size(); i1<iend1; i1++)
    {
        ...
        int bestDist = INT_MAX;		//白，第一小
        int bestDist2 = INT_MAX;	//白，第二小
        int bestIdx2 = -1;
        ...
        if(bestDist<=TH_LOW)
        {
	    //白,如果最佳匹配值与第二佳匹配值大很多,才是真正的匹配成功.这种思路的原理参考SearchByBoW()步骤4
            if(bestDist<(float)bestDist2*mfNNratio)
            {
		//白，如果已经有对应匹配，则删除？，只取最后那次匹配结果
    ...
    //白，朝向检测的思路是:认为整体特征点的角度变化是趋于一个统一的角度，对所有已匹配特征点的角度变换进行统计，只认可排序前三的三组匹配。
    if(mbCheckOrientation)
    {
        int ind1=-1;
        int ind2=-1;
	...
	//白，将除这三个方向的匹配对之外的匹配，否定
		...
		//白，没有对vnMatches21进行删除匹配操作，因为之后就没有使用vnMatches21。
...
int ORBmatcher::SearchByProjection(Frame &CurrentFrame, const Frame &LastFrame, const float th, const bool bMono)
{
    ...
    // Rotation Histogram (to check rotation consistency),HISTO_LENGTH=30
    vector<int> rotHist[HISTO_LENGTH];
    for(int i=0;i<HISTO_LENGTH;i++)
        rotHist[i].reserve(500);
    const float factor = 1.0f/HISTO_LENGTH;	//白，此处有BUG,应该是factor = HISTO_LENGTH / 360.0f,左侧的语句使得范围为[0,round(360/30)]，而不是[0,29]。
    ...
    // 判断前进还是后退
    //白，与基线进行大小判断前进、后退的依据是什么？这里的前进、后退标志量是为标志点候选搜索范围的设定服务的，相当于划分了三个区域，每个区域对应一个搜索条件，不是仅仅判断前进/后退（两个结果区域）。变量命名有一定歧义。
    ...
    for(int i=0; i<LastFrame.N; i++)
    {
		...
		//白，此处有可疑BUG，此处判断可以上移，且判断x3Dc(2)是否为0
                if(invzc<0)
                    continue;
		    ...
		    //白，目的是为当前帧特征点找到与之对应的MapPoint，当已经有时就不需要匹配了。因为上一帧对应的MapPoints中每一个都对应一个搜索区域，区域有重合。
                    const size_t i2 = *vit;
                    if(CurrentFrame.mvpMapPoints[i2])
                        if(CurrentFrame.mvpMapPoints[i2]->Observations()>0)
                            continue;

		    //白，单目的情况，mvuRight[*]为-1
