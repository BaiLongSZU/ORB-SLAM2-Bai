...
bool LoopClosing::DetectLoop()
{
    ...
    //白，根据最小相关值得出的关键帧为闭环候选帧vpCandidateKFs[i]，每一帧闭环候选帧的相连关键帧组成的队列为子候选组spCandidateGroup。第一次时会将所有子候选组存入当前闭环候选组vCurrentConsistentGroups。上一闭环候选组的自候选组称为子连续组，第二+次时会首先将当前子候选组中的每一帧与上一闭环候选组的所有帧进行匹配，如果已经有记录则
    ...
    for(size_t i=0, iend=vpCandidateKFs.size(); i<iend; i++)
    {
        ...
        for(size_t iG=0, iendG=mvConsistentGroups.size(); iG<iendG; iG++)
        {
		    ...
		    //白，闭环候选组的插入是以闭环候选帧为主索引，所以防止多次插入的标志量应以闭环候选帧序列vpCandidateKFs为准，索引值应为vbConsistentGroup[i]，不是vbConsistentGroup[iG]。
		    //白，ConsistentGroup.second是什么含义？该相连组（通过关键帧的相连帧关系）连续出现的次数？这就是”子连续组“所谓的连续性？什么用途？
                {
