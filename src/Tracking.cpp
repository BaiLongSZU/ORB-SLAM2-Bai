        ...
        {            
            ...
            else	//白，此时mState==OK
            {
                ...
                else
                {
                    ...
                    if(!mVelocity.empty())
                    {
                        bOKMM = TrackWithMotionModel();		//白，会根据已匹配的点数修改mbVO。
//                         // 这三行没啥用？
...
void Tracking::CreateInitialMapMonocular()
{
	...
	//白，将关键帧KeyFrame与地图点MapPoint关联起来，联系是特征点KeyPoint
        pKFini->AddMapPoint(pMP,i);
        pKFcur->AddMapPoint(pMP,mvIniMatches[i]);

        // a.表示该MapPoint可以被哪个KeyFrame的哪个特征点观测到
	//白，将地图点MapPoint和关键帧KeyFrame关联起来
        pMP->AddObservation(pKFini,i);
        pMP->AddObservation(pKFcur,mvIniMatches[i]);

        // b.从众多观测到该MapPoint的特征点中挑选区分读最高的描述子
	//白，初始化时同一个特征点的描述子最多有两个，因为只有两帧图像，所以取中值描述子的意义不大
...
bool Tracking::TrackReferenceKeyFrame()
{
    ...
    // 步骤4:通过优化3D-2D的重投影误差来获得位姿（只优化Frame的Tcw，不优化MapPoints的坐标，会计算mvbOutlier）
    ...
    for(vector<MapPoint*>::iterator vit=mCurrentFrame.mvpMapPoints.begin(), vend=mCurrentFrame.mvpMapPoints.end(); vit!=vend; vit++)
    {
        ...
        if(pMP)
        {
            ...
            else
            {
                ...
                // 标记该点被当前帧观测到
                pMP->mnLastFrameSeen = mCurrentFrame.mnId;
                // 标记该点将来不被投影，因为已经匹配过，	白，mbTrackInView有什么用途吗？表示该MapPoint不在视野范围内，不可被观察到。
                pMP->mbTrackInView = false;
            }
        }
    }
...
void Tracking::UpdateLocalKeyFrames()
{
    ...
    // 步骤1：遍历当前帧的MapPoints，记录所有能观测到当前帧MapPoints的关键帧  白，并统计该关键帧与当前帧共视MapPoint的数量
    ...
    for(map<KeyFrame*,int>::const_iterator it=keyframeCounter.begin(), itEnd=keyframeCounter.end(); it!=itEnd; it++)
    {
        ...
        // mnTrackReferenceForFrame防止重复添加局部关键帧，	白，mvpLocalKeyFrames里边存储的每一个关键帧，都会设置mnTrackReferenceForFrame，这个变量指向当前帧表示已经被链接，当下次查询到该帧时，可以判断是否已经被链接来决定是否将该帧存储在mvpLocalKeyFrames中。
